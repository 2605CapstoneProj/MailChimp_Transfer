...
const CONFIG_MC = {
  ...
  COL_NAME:    'Contact Name',        // single full-name column → maps to NAME in MC
  COL_BUSNAME: 'Business Name',     // set to null if there is no company column
...
function syncToMailchimp(filters, campaignTag) {
  ...
  contacts.forEach(contact => {
    try {
      ...
      if (contact.name)    payload.merge_fields.NAME   = contact.name;
      if (contact.company) payload.merge_fields.BUSNAME = contact.company;
...
function testFindContact(email) {
  ...
  return {
    ...
    status:   body.status,
    name:     body.merge_fields?.NAME || '—',
    company:  body.merge_fields?.BUSNAME || '—',
...
function testAddContact(email, name) {
  ...
  const payload = {
    ...
    status_if_new: 'subscribed',
    merge_fields:  { NAME: name || 'Test Contact' },
  };
  const res  = UrlFetchApp.fetch(
    `https://${dc}.api.mailchimp.com/3.0/lists/${CONFIG_MC.MC_LIST_ID}/members/${hash}`,
    {
